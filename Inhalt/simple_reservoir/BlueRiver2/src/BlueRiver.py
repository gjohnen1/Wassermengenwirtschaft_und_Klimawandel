...
# Jupyter may pass an inline backend that is not available in the RTC venv.
...
try:
    ...
    from rtctools_interface.optimization.plot_goals_mixin import PlotGoalsMixin as PlotMixin
...
class RateOfChangeGoal(Goal):
    ...
    def __init__(self, optimization_problem, target_min, target_max, variable, priority, weight, function_range, function_nominal=1):
        ...
        self.order = 1  # use 2 to let the square be minimized (= RTC default)
        # NOTE: change order to 4 or 6 (Hessian not constant!!!) to improve accuracy of this target
...
class BlueRiver(PlotMixin, GoalGeneratorMixin, GoalProgrammingMixin, CSVMixin, ModelicaMixin, CollocatedIntegratedOptimizationProblem):
...
if __name__ == "__main__":
    run_optimization_problem(BlueRiver, log_level=logging.INFO, plotting_library="matplotlib") #"plotly or matplotlib"
